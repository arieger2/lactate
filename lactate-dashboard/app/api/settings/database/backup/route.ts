  ...
  try {
    ...
    switch (type) {
      case 'full':
        ...
        command = `pg_dump -h ${dbHost} -p ${dbPort} -U ${dbUser} -d ${database} -F p -f "${filename}"`
        break
      case 'compressed':
        filename = `${targetDir}/laktat_compressed_${timestamp}.sql.gz`
        command = `pg_dump -h ${dbHost} -p ${dbPort} -U ${dbUser} -d ${database} -F p | gzip > "${filename}"`
        break
      case 'data-only':
        filename = `${targetDir}/laktat_data_only_${timestamp}.sql`
        command = `pg_dump -h ${dbHost} -p ${dbPort} -U ${dbUser} -d ${database} -a -F p -f "${filename}"`
        break
      case 'table':
        filename = `${targetDir}/laktat_${table}_${timestamp}.sql`
        command = `pg_dump -h ${dbHost} -p ${dbPort} -U ${dbUser} -d ${database} -t ${table} -F p -f "${filename}"`
        break
    }
